...
###PCA INterpretation (based on loadinsg table###

# Dim.1 (Principal Component 1)
# High Positive Loadings:
#   Moisture content (0.356)
# Particle sizes: 6.3 mm (0.256), 2.7 mm (0.284), 2.36 mm (0.325), 1.18 mm (0.330), 600 μm (0.257)
# High Negative Loadings:
#   Slope (-0.369)
# Total water depth (-0.332)
# Particle size: 150 μm (-0.355)
# Interpretation:
#
#   This component primarily contrasts the physical characteristics of the soil
# such as moisture content and certain particle sizes with the slope
# water depth. The positive loadings signify that higher moisture and
# larger particle sizes are associated, whereas higher slopes and
# deeper water layers are associated in the opposite direction.

# Dim.2 (Principal Component 2)
# High Positive Loadings:
#   Particle sizes: 45 μm (0.442), 75 μm (0.439), 90 μm (0.451)
# High Negative Loadings:
#   Particle size: 300 μm (-0.511)
# Interpretation:
#
#   This dimension seems to capture the contrast between finer particles (45 μm, 75 μm, 90 μm) and a relatively larger particle size (300 μm). It may indicate a gradient of particle size where finer particles are positively correlated and larger particles are inversely related.
# Dim.3 (Principal Component 3)
# High Positive Loadings:
#   Slope (0.328)
# Width (0.341)
# Particle sizes: 1.18 mm (0.314), 600 μm (0.318)
# High Negative Loadings:
#   Moisture content (-0.284)
# Particle sizes: 45 μm (-0.226)
# Interpretation:
#
#   Dim.3 captures a contrast between larger slope, width, and medium-sized particles against moisture content and finer particles, suggesting that areas with higher slopes and widths tend to have larger particle sizes.
# Dim.4 (Principal Component 4)
# High Positive Loadings:
#   Distance (0.565)
# Total nest depth (0.631)
# High Negative Loadings:
#   Particle size: 300 μm (-0.292)
# Interpretation:
#
#   Dim.4 differentiates between variables related to distance and total nest depth versus particle size of 300 μm. It might suggest that locations with greater distances and nest depths have smaller amounts of this specific particle size.
# Dim.5 (Principal Component 5)
# High Positive Loadings:
#   pH (0.715)
# Particle size: 6.3 mm (0.348)
# High Negative Loadings:
#   Distance (-0.315)
# Particle size: 90 μm (-0.187)
# Interpretation:
#
#   Dim.5 captures the contrast between pH and larger particle sizes versus distance and finer particles. It indicates that areas with higher pH levels are associated with larger particles, while areas that are farther away tend to have finer particles.
# Dim.6 (Principal Component 6)
# High Positive Loadings:
#   Width (0.735)
# Total nest depth (0.413)
# High Negative Loadings:
#   Total water depth (-0.381)
# Interpretation:
#
#   This dimension shows a contrast between the width and nest depth against total water depth. It suggests that wider areas with deeper nests correlate negatively with total water depth, implying that these areas might be less waterlogged.
# Summary
# Overall, the PCA highlights key groupings of variables based on their physical characteristics and environmental measures.
# Particle sizes are highly influential across multiple dimensions, showing significant contrasts between finer and coarser particles.
# Environmental variables like slope, moisture content, and pH are also crucial, with their influence varying across different dimensions.
# The relationships identified can help in understanding how different environmental factors and soil properties co-vary, providing insights for further ecological or geological analysis.
...
location_indexes <- as.integer(factor(analysis_data_agg$location)) # note not used in final model
...
stan_data <- list(
  ...
  J = length(unique(location_indexes)), # Number of unique locations
  location = location_indexes, # Index of locations for random effects
...
### Note earlier STan model showed no sig effect od dims 2 to 6, so these removed and refitted. LOOIC lower for reduced model
 ...
 # fit_fe <- stan(
 #   model_code = stan_model_code_no_location,  # Stan model code
 #   data = stan_data,
...
# saveRDS(fit_fe, file = "turtle_nest_fit_all_dimensions_no_locations_weighted.rds")

# Load the fit object from the file
fit <- readRDS("turtle_nest_fit_fe_dim1_only.rds")
...
## Only Dim 1 was significant in its effect on nesting
...
# Therefore, site selection is positively associated with higher slopes,
#  deeper water and smaller particle size, and egatively associated with moisture
# content
...
### Unusued alternative model code for reference:
...
stan_model_code_re <- "
data {
  int<lower=0> N; // Number of observations or data points (i.e., sites).
  int<lower=1> J; // Number of sandbanks where observations were made.
  int<lower=1, upper=J> location[N]; // Index for each observation indicating its sandbank.
  int<lower=0, upper=1> y[N]; // Binary outcome for each observation (1 for success, 0 for failure).
  int<lower=1> K; // Number of different characteristics measured (covariates).
  matrix[N, K] x_mean; // Average values of the characteristics for each observation.
  matrix<lower=0>[N, K] x_sd; // Uncertainty in the measurements of each characteristic.
}
parameters {
  real alpha; // Baseline effect not related to any specific characteristic (i.e., when all variables are at mean).
  vector[K] beta; // Effects of each characteristic on the outcome (the slope parameter for each variable).
  vector[J] z; // Variability in outcomes related to each sandbank.
  real<lower=0> sigma; // Overall scale of variability across sandbanks.
}
transformed parameters {
  vector[J] location_effect; // Effects specific to each sandbank.
  location_effect = z * sigma; // Calculate these sandbank effects by scaling variability to set from unitless values to log-odds units.
}
model {
  vector[N] eta; // Linear predictor for each observation.
  eta = alpha + x_mean * beta; // Efficient matrix-vector multiplication for covariates.

  for (n in 1:N) { // Loop to include measurement uncertainty adjustments and location effects.
    real adjustment = 0;

    // Measurement uncertainty adjustment.
    for (k in 1:K) {
      adjustment += 0.5 * beta[k] * beta[k] * square(x_sd[n, k]);
    }

    eta[n] += location_effect[location[n]] + adjustment; // Add location-specific effect and adjustment.
  }
...
generated quantities {
  ...
  for (n in 1:N) {
    ...
    // Recalculate the adjustment term for generated quantities.
    for (k in 1:K) {
      adjustment += 0.5 * beta[k] * beta[k] * square(x_sd[n, k]);
    }

    // Recompute linear predictor.
    real eta_n = alpha + dot_product(to_vector(x_mean[n, ]), beta) + location_effect[location[n]] + adjustment;
...
data {
  int<lower=0> N;                     // Number of observations (sandbanks)
  int<lower=1> J;                     // Number of sandbanks
  int<lower=1, upper=J> location[N];  // Sandbank indices for observations
  int<lower=0, upper=1> y[N];         // Binary outcome (1 for nesting, 0 for no nesting)
  int<lower=1> K;                     // Number of covariates
  matrix[N, K] x_mean;                // Mean values of covariates for each sandbank
  matrix<lower=0>[N, K] x_sd;         // Standard deviations of covariates
  matrix<lower=1>[N, K] n_obs;        // Sample sizes for each covariate (inverse variance weighting)
}
parameters {
  real alpha;                         // Overall intercept
  vector[K] beta;                     // Coefficients for covariates (one for each covariate)
  vector[J] gamma;                    // Fixed effect for each sandbank (one for each sandbank)
}
model {
  vector[N] eta;                      // Linear predictor
  eta = alpha + x_mean * beta + gamma[location]; // Add fixed effects for sandbanks

  for (n in 1:N) {
    real adjustment = 0;

    // Measurement uncertainty adjustment with inverse variance weighting
    for (k in 1:K) {
      real variance = square(x_sd[n, k]) / n_obs[n, k]; // Variance of the mean
      adjustment += 0.5 * beta[k] * beta[k] * variance; // Weighted adjustment
    }

    eta[n] += adjustment;             // Update eta with uncertainty adjustment
  }
  ...
  // Priors
  alpha ~ normal(0, 10);              // Prior for the overall intercept
  beta ~ normal(0, 1.5);              // Priors for covariate coefficients
  gamma ~ normal(0, 1);               // Regularizing prior for sandbank-specific effects
}
generated quantities {
  ...
  for (n in 1:N) {
    ...
    // Recalculate the uncertainty adjustment for each covariate
    for (k in 1:K) {
      real variance = square(x_sd[n, k]) / n_obs[n, k]; // Variance of the mean
      adjustment += 0.5 * beta[k] * beta[k] * variance; // Weighted adjustment
    }

    // Recompute the linear predictor for this observation
    real eta_n = alpha + dot_product(to_vector(x_mean[n, ]), beta) + gamma[location[n]] + adjustment;

    // Calculate the log-likelihood for this observation
    log_lik[n] = bernoulli_logit_lpmf(y[n] | eta_n);
  }
}
"
